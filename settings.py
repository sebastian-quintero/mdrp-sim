...
SIMULATE_FROM: time = time(8, 0, 0)  # Simulate from this time on
SIMULATE_UNTIL: time = time(8, 59, 59)  # Simulate until this time
CREATE_USERS_UNTIL: time = time(9, 15, 0)  # Create new users to submit orders until this time
...
COURIER_MOVEMENT_PROBABILITY: float = 0.05  # Probability that a courier WILL move
...
DISPATCHER_PROSPECTS_MAX_DISTANCE: float = 3  # Maximum distance [km] between a courier and a store to be considered prospect
DISPATCHER_PROSPECTS_MAX_ORDERS: float = 3  # Maximum number of orders a courier can carry simultaneously

ORDER_MAX_PICK_UP_SERVICE_TIME: float = min_to_sec(10)  # Maximum service time [sec] at the pick up location
ORDER_MAX_DROP_OFF_SERVICE_TIME: float = min_to_sec(5)  # Maximum service time [sec] at the drop off location
ORDER_MIN_SERVICE_TIME: float = min_to_sec(2)  # Minimum service time [sec] at either a pick up or drop off location
...
DISPATCHER_MATCHING_POLICY: str = 'greedy'  # Policy for matching orders and couriers. Options: ['greedy']
DISPATCHER_PREPOSITIONING_POLICY: str = 'naive'  # Policy for prepositioning couriers. Options: ['naive']
DISPATCHER_PREPOSITIONING_TIMING_POLICY: str = 'fixed'  # Policy for the prepositioning timing. Options: ['fixed']
